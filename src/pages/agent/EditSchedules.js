...
import { getBusForAgent } from '../../redux/actions/BusActions'
...
const EditSchedules = props => {
  ...
  const Submit = e => {
    ...
    //   }
    // })
    props.updateSchedules(props.match.params.id, data)
    console.log(data)
  }
  useEffect(() => {
    props.loadRoutesWithID()
    props.getBusForAgent()
    props.getSchedulesById(props.match.params.id)
  }, [])
  return (
    <Container>
      <Row>
          ...
          <Card body>
            <Form onSubmit={Submit}>
                ...
                <Col sm={6}>
                  <Select
                    value={
                      props.routes &&
                      props.singleData &&
                      props.routes.filter(option => option.value === `${props.singleData.route_id}`)
                    }
                ...
                <Col sm={6}>
                  <Select
                    ...
                    value={
                      props.buses &&
                      props.singleData &&
                      props.buses.filter(option => option.value === `${props.singleData.bus_id}`)
                    }
                  />
                </Col>
              </FormGroup>
              <FormGroup row>
                ...
                <Col sm={3}>
                  <DatePicker
                    selected={props.singleData && new Date(props.singleData.date)}
...
const mapStateToProps = state => {
  return {
    ...
    buses: state.busData.data,
    singleData: state.schedules.singleData
  }
}
